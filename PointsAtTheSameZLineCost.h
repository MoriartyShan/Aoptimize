...
private:
  ...
  cv::Matx33d _cameraK;
public:
  static ceres::CostFunction* CreateCompact(const cv::Matx33d& cameraK, const std::vector<cv::Point2d> &_points) {
  ...
  static ceres::CostFunction* Create(const cv::Matx33d& cameraK, const std::vector<cv::Point2d> &_points) {
    return (new ceres::AutoDiffCostFunction<ZLine, 1, 3, 1>(
        new ZLine(cameraK, _points)));
  }
...
//      const int axis) {
//    return (new ceres::AutoDiffCostFunction<AxisLine, 1, 4>(
